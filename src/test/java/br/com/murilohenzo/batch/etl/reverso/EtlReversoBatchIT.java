...
import br.com.murilohenzo.batch.etl.reverso.validator.JobParametersInvalidException;
...
class EtlReversoBatchIT {
    ...
    void explainUsesGovernedColumns() {
        d0();
        assertThat(jdbc.queryForList("EXPLAIN ANALYZE " + GovItemReader.SELECT + GovItemReader.FROM
                + GovItemReader.WHERE + " ORDER BY s.cdServicoGovernanca LIMIT 3", String.class)).isNotEmpty();
    }
    ...
    void volumeReadsFiftyThousandThenOnlySixThousandTwoHundredChanges() throws Exception {
        ...
        var plan = jdbc.queryForList("EXPLAIN ANALYZE " + GovItemReader.SELECT + GovItemReader.FROM
                + GovItemReader.WHERE + " ORDER BY s.cdServicoGovernanca LIMIT 1000", String.class);
    ...
    private JobExecution runVolume() throws Exception {
        ...
        var reader = new GovItemReader(dataSource, 1000);
    ...
    private List<String> eligible() {
        return jdbc.query(GovItemReader.SELECT + GovItemReader.FROM + GovItemReader.WHERE + " ORDER BY s.cdServicoGovernanca",
